import{f as q,b as D,s as f,o as O,N as I,P as B}from"./tui-pagination-5a898e46.js";const l=document.querySelector(".carousel-cr-by"),F=document.querySelector(".cr-by-arrow-left"),W=document.querySelector(".cr-by-arrow-right"),j=l.querySelector(".cr-by-card").offSetWidth;[...l.children];let h=!1,E,x;Math.round(l.offSetWidth/j);const K=()=>{l.scrollLeft-=247},J=()=>{l.scrollLeft+=247},U=t=>{h=!0,l.classList.add("dragging"),E=t.pageX,x=l.scrollLeft},Y=t=>{h&&(l.scrollLeft=x-(t.pageX-E))},X=()=>{h=!1,l.classList.remove("dragging")};l.addEventListener("mousedown",U);l.addEventListener("mousemove",Y);document.addEventListener("mouseup",X);F.addEventListener("click",K);W.addEventListener("click",J);const k="favoriteCocktails",A=document.querySelector(".cardlist"),M="cocktails/lookup/",P="id",n=JSON.parse(localStorage.getItem(k))??[];let u,r;const R={disabledScroll(){document.body.style.cssText=`
    overflow: hidden;
    padding-right: ${window.innerWidth-document.body.offsetWidth}px;`},enabledScroll(){document.body.style.cssText=""}};function z(){const t=document.querySelector(".go-top-btn");if(window.innerWidth>=1280)t.style.right="40px";else return}function V(){const t=document.querySelector(".go-top-btn");t.style.right="30px"}A.addEventListener("click",G);A.addEventListener("click",tt);async function G(t){if(!t.target.classList.contains("cardlist-learn"))return;r=t.target.closest(".cardlist-item");const o=t.target.closest(".cardlist-item").dataset.id;await q(M,P,o).then(a=>{const{_id:s,drinkThumb:i,instructions:c,drink:e,ingredients:S,description:L}=a[0];u={_id:s,drinkThumb:i,description:L,drink:e};let v=S.map(g=>`<li data-id="${g.ingredientId}" class="text item-card"><a href="#" class="ingredient-btn">${g.measure||""}${g.title}</a></li>`).join("");Q(o,v,e,c,i),R.disabledScroll(),z()})}function Q(t,o,a,s,i){D.create(`<div class="container-popup id-for-del" data-id="${t}">

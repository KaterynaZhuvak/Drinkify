import{f as w,b as P,s as u,o as R,N as C,P as H}from"./tui-pagination-00138ec7.js";const k="favoriteCocktails",I=document.querySelector(".cardlist"),q="cocktails/lookup/",x="id",l=JSON.parse(localStorage.getItem(k))??[];let d,n;const A={disabledScroll(){document.body.style.cssText=`
    overflow: hidden;
    padding-right: ${window.innerWidth-document.body.offsetWidth}px;`},enabledScroll(){document.body.style.cssText=""}};I.addEventListener("click",N);I.addEventListener("click",B);async function N(t){if(!t.target.classList.contains("cardlist-learn"))return;n=t.target.closest(".cardlist-item");const a=t.target.closest(".cardlist-item").dataset.id;await w(q,x,a).then(o=>{const{_id:i,drinkThumb:s,instructions:c,drink:e,ingredients:y,description:b}=o[0];d={_id:i,drinkThumb:s,description:b,drink:e};let f=y.map(m=>`<li data-id="${m.ingredientId}" class="text item-card"><a href="#" class="ingredient-btn">${m.measure||""}${m.title}</a></li>`).join("");O(a,f,e,c,s),A.disabledScroll()})}function O(t,a,o,i,s){P.create(`<div class="container-popup id-for-del" data-id="${t}">
  ...
  <button type="button" class="button-card favorite add-to-fav-cockt" data-id="${t}">
   ...
   <button type="button" class="button-card back close-cocktail-modal-back">Back</button></div>
        ...
        </li>`}const v=document.querySelector(".cardlist"),$=document.querySelector(".plug"),p=document.querySelector(".pagination-main"),J="favoriteCocktails",g=JSON.parse(localStorage.getItem(J))??[];let Y=1,r=6,T=4;screen.width>=1280&&(T=7);(window.location.pathname==="/Drinkify/favorite-cocktails.html"||window.location.pathname==="/favorite-cocktails.html")&&h(g,v);function h(t,a){(window.location.pathname==="/Drinkify/favorite-cocktails.html"||window.location.pathname==="/favorite-cocktails.html")&&(t.length?$.classList.add("visually-hidden"):$.classList.remove("visually-hidden")),a.innerHTML="",p.innerHTML="",t.length<=r?a.innerHTML=t.map(o=>K(o,j)).join(""):(_(t,a,r,Y),W(t,p,r))}function _(t,a,o,i){a.innerHTML="",i--;let s=o*i,c=s+o,e=t.slice(s,c);return h(e,v)}function W(t,a,o){a.innerHTML="";const i={totalItems:t.length,itemsPerPage:o,visiblePages:T,page:1,centerAlign:!1,firstItemClassName:"tui-first-child",lastItemClassName:"tui-last-child",template:{page:'<a href="#" class="tui-page-btn btnStyle">{{page}}</a>',currentPage:'<strong class="tui-page-btn tui-is-selected btnStyleActive">{{page}}</strong>',moveButton:'<a href="#" class="tui-page-btn tui-{{type}} btnStyle"><span class="tui-ico-{{type}}">{{type}}</span></a>',disabledMoveButton:'<span class="tui-page-btn tui-is-disabled tui-{{type}} btnStyle"><span class="tui-ico-{{type}}">{{type}}</span></span>',moreButton:'<a href="#" class="tui-page-btn tui-{{type}}-is-ellip"><span class="tui-ico-ellip">...</span></a>'}};new H(p,i).on("beforeMove",c=>{const{page:e}=c;_(g,v,r,e)})}export{K as c,U as i};
